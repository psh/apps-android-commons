...
class CategoryContentProviderUnitTest {
    private lateinit var contentProvider: CategoryContentProvider
    ...
    fun setUp() {
        ...
        contentProvider = CategoryContentProvider()
    ...
    fun testQuery() {
        ...
        contentProvider.query(CategoryContentProvider.BASE_URI, null, null, null, null)
    }

    @Test
    fun testInsert() {
        `when`(database.insert(any(), any(), any())).thenReturn(1L)
        contentProvider.insert(CategoryContentProvider.BASE_URI, mock(android.content.ContentValues::class.java))
    }

    @Test
    fun testUpdate() {
        `when`(database.update(any(), any(), any(), any(), any())).thenReturn(1)
        val uri = CategoryContentProvider.uriForId(1)
    ...
    fun testBulkInsert() {
        `when`(database.insert(any(), any(), any())).thenReturn(1L)
        contentProvider.bulkInsert(CategoryContentProvider.BASE_URI, arrayOf(mock(android.content.ContentValues::class.java)))
    }
}
